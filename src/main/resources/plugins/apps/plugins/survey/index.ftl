	...
	<div id="userReportsId">
	    ...
	    <#assign size=model.userReportsList?size - userReports?counter>
			<div class="item-box item-bar bgrow" 
				onclick="selectUserReports(this, 'testId', '2008/12/01');">
				<div class="contents">
					<h4 class="media-heading">Workday</h4>
				</div>
				<div class="bar" style="width:50%; background-color: rgba(255, 233, 216, 0.${size});"></div>
				<input type="hidden" name="urNo" value=""/>
			</div>
		</#list>
		</#if>
		</#if>
